...
def plot_pair(data, ax):
	...
	for house_idx, house_data in enumerate(histo_matrix) :
		...
		for feature_idx, feature_data in enumerate(histo_matrix[house_idx]):
			for feature_idx_mapped, feature_data_mapped in enumerate(histo_matrix[house_idx]):
				model_idx = feature_idx + feature_idx_offset
				model_idx_mapped = feature_idx_mapped + feature_idx_offset

				if feature_idx == feature_idx_mapped:
					y_values = feature_data
					hist, bins = generate_historgram(y_values)
					center = get_centers(bins)
					width = 1 * (bins[1] - bins[0])
					ax[feature_idx][feature_idx_mapped].bar(center, hist, width=width, color=house_color, alpha=0.5)
				else:
					x_values = feature_data
					y_values = feature_data_mapped
					
					ax[feature_idx][feature_idx_mapped].scatter(x_values, y_values, color=house_color, alpha=0.6)

				print(f"{house_name}: {DATA_MODEL[model_idx]['name']} vs {DATA_MODEL[model_idx_mapped]['name']}") 

	# set metadata
	for feature_idx, feature_data in enumerate(histo_matrix[0]):
			for feature_idx_mapped, feature_data_mapped in enumerate(histo_matrix[0]):
				model_idx = feature_idx + feature_idx_offset
				model_idx_mapped = feature_idx_mapped + feature_idx_offset

				feature_name = DATA_MODEL[model_idx]['name']
				feature_name_mapped = DATA_MODEL[model_idx_mapped]['name']
				ax[feature_idx][feature_idx_mapped].set_xlabel(feature_name)
				ax[feature_idx][feature_idx_mapped].set_ylabel(feature_name_mapped)
				# ax[feature_idx][feature_idx_mapped].legend(HOUSES)
...
def main():
	...
	fig, ax = plt.subplots(13, 13, figsize=(50, 50))
